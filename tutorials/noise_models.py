"""
# Tutorial - Noise Models

In this tutorial, we will modify our quantum-Reinforce implementation for discrete action-spaces ```reinforce_quantum.py``` to include noise models in the state-vector simulation.

In the original script, we use a noiseless state-vector simulator. However, quantum hardware suffers from noise. In this tutorial, we include some noise models in our state-vector simulation to bridge the gap between simulation and real hardware. 
"""

# Let's start by importing all relevant libraries
...
"""
Now comes the difference. We need to change the ```parametrized_quantum_circuit``` function to include noise models.

In particular, let's add depolarizing noise after every layer with a probability ```depolarizing_p``` which is a new parameter in the config file. This corresponds to a single line of code using ```qml.DepolarizingChannel```.

Depolarizing noise is a common noise model that simulates the effect of qubit decoherence. It affects a quantum state by replacing it with a mixed state with probability p, or leaving it unchanged otherwise.

Pennylane has many more noise models available that can easily be added to this function for more complex and nuanced noise models.

"""

# QUANTUM CIRCUIT: define your ansatz here:
def parameterized_quantum_circuit(
    x, input_scaling, weights, num_qubits, num_layers, num_actions, observation_size, depolarizing_p
):
...
"""
We also have to slightly change the quantum agent to receive the depolarizing_p parameter and pass it to the quantum circuit.
"""
...
class ReinforceAgentQuantum(nn.Module):
    def __init__(self, observation_size, num_actions, config):
        ...
        self.depolarizing_p = config["depolarizing_p"]
...
"""
The rest of the code is unchanged and copied from the original ```reinforce_quantum.py``` script.

The only difference comes at the end of the script where we add the ```depolarizing_p``` parameter to the config file.
"""
...
if __name__ == "__main__":
    ...
    """
    The config must now also receive the ```depolarizing_p``` parameter.

    Furthermore, the device must be changed to ```"default.mixed"``` due to the noise channel and the differentiation method must be set to ```"backprop"```.
    """
    ...
    class Config:
        ...
        # Noise parameters that must be added
        depolarizing_p: float = 0.1 # The probability of the depolarizing channel
        ...
        device: str = "default.mixed"  # Quantum device
