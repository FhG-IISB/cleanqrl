...
class JumanjiWrapperKnapsack(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if truncate:
            values = state["values"] # self.previous_state["values"]
            weights = state["weights"] # self.previous_state["weights"]
...
def hamiltonian_encoding_ansatz(x, input_scaling, weights, num_qubits, num_layers, num_actions):
    ...
    for layer in range(num_layers):
        ...
        for idx_h in range(num_qubits):  # single-qubit terms
            qml.RX(input_scaling[layer] * h[:, idx_h], wires=idx_h)

        idx_J = 0
        for i in range(num_qubits):
            for j in range(i + 1, num_actions):
                # qml.CNOT(wires=[i, j])
                qml.RX(input_scaling[layer] * J[:, idx_J], wires=j)
                # qml.CNOT(wires=[i, j])
                idx_J += 1
